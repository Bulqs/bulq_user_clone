...
// --- CRITICAL FIX 1: Import the consolidation verify action ---
...
function PaymentVerifyContent() {
    ...
    // --- CRITICAL FIX 2: Grab the flow type ---
    ...
    useEffect(() => {
        ...
        const verify = async () => {
            try {
                ...
                // --- CRITICAL FIX 3: Dynamic API Routing ---
                if (flowType === 'consolidation') {
                    console.log("Verifying CONSOLIDATION Payment for:", bookingId, "Ref:", paymentRef);
                    response = await verifyConsolidationPaymentStatus(bookingId, paymentRef);
                } else {
                    console.log("Verifying STANDARD Payment for:", bookingId);
                ...
                if (isSuccessStatus && !isNaN(finalAmount) && finalAmount > 0) {
                    ...
                    setBookingDetails({
                        ...
                        isConsolidation: flowType === 'consolidation' // Tag it for the UI
                    });
                } else {
                    setVerified(false);
        ...
        verify();
    }, [bookingId, paymentRef, flowType]); // Added flowType to dependencies
    ...
    return (
        <div className="min-h-screen bg-slate-100 py-8 px-4 flex flex-col items-center">
            ...
            <div ref={receiptRef} className="bg-white w-full max-w-[800px] rounded-3xl shadow-xl overflow-hidden relative border border-gray-200">
                ...
                {/* --- CRITICAL FIX 4: Hide Policy for Consolidations --- */}
